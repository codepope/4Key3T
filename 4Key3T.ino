...
#define Y_DIM 1  //number of rows of keys
...
// this example is just two, one on top of another to make a 1x4 grid
...
NeoKey1x4Callback blink(keyEvent evt) {
  ...
  if (evt.bit.EDGE == SEESAW_KEYPAD_EDGE_RISING) {
    ...
    Serial.println(key);
    mykeystroke=keys[key];
    if(mykeystroke.control) {
      Keyboard.press(MOD_LEFT_CTRL);
    }
    if(mykeystroke.command) {
      Keyboard.press(KEY_LEFT_GUI);
    }
    Keyboard.print(mykeystroke.key);
    Keyboard.releaseAll();
